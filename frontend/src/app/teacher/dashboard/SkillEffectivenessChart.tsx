...
export function useSkillEffectivenessOption(items: SkillEffectivenessItem[]) {
    return useMemo(() => {
        const labels = items.map(item => shortSkillLabel(item.skill_id));

        const series = CHART_DIMENSIONS.map(dim => ({
            name: dim.label,
            type: 'bar' as const,
        ...
        return {
            tooltip: {
                ...
                formatter: (params: Array<{ seriesName: string; value: number; marker: string; dataIndex: number }>) => {
                    const skillIdx = params[0]?.dataIndex ?? 0;
...
export default function SkillEffectivenessChart({ items }: SkillEffectivenessChartProps) {
    const option = useSkillEffectivenessOption(items);
