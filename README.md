...
│                      │                 │  (App Router)        │
│  JWT Auth + RBAC     │                 │                      │
│  KA-RAG Pipeline     │                 │  Teacher Dashboard   │
│  Agent Orchestrator  │                 │  Student Dialogue    │
│  WebSocket Streaming │                 │  ECharts Analytics   │
└──┬────┬────┬────┬────┘                 └──────────────────────┘
   │    │    │    │
   ▼    ▼    ▼    ▼
┌────┐┌────┐┌────┐┌────────┐
│ PG ││Neo4││Redis││ Ollama │
│    ││j   ││     ││        │
│pgv.││    ││     ││qwen2.5 │
└────┘└────┘└────┘└────────┘
```
...
role-based access control (SYS_ADMIN, SCHOOL_ADMIN, TEACHER, STUDENT).

**Frontend** (Next.js 16): React 19 with App Router, CSS Modules, ECharts for
analytics dashboards, WebSocket for real-time AI dialogue streaming.
...
RRF hybrid retrieval (pgvector + Neo4j), and BKT-driven scaffold fading.
...
internal/
  ...
  domain/model/                    # GORM models (user, knowledge, session, skill)
...
plugins/skills/                    # Pluggable skill definitions (Socratic, etc.)
...
Connect to `/api/v1/sessions/:id/stream` with a Bearer token.
